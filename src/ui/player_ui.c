...
#ifdef __APPLE__
        /* ──────────────────────────────────────────────────────────────
           Push the current track info into SketchyBar (macOS only)
        ────────────────────────────────────────────────────────────── */
        {
                char timebuf[32];
                double duration = get_current_song_duration();
                if (duration <= 0.0)
                        duration = 1.0;
                int el_min = (int)(elapsed_seconds / 60);
                int el_sec = (int)elapsed_seconds % 60;
                int tot_min = (int)(duration / 60);
                int tot_sec = (int)duration % 60;
                snprintf(timebuf, sizeof(timebuf), "%d:%02d/%d:%02d", el_min,
                         el_sec, tot_min, tot_sec);
                ...
                if (songdata && songdata->metadata) {
                        ...
                        if (current_song != NULL) {
                                snprintf(sb_cmd, sizeof(sb_cmd),
                                         "/opt/homebrew/bin/sketchybar --set widgets.media label=\"􀑪  %s [%s]\"",
                                         current_song->song.file_path, timebuf);
                        } else {
                                snprintf(sb_cmd, sizeof(sb_cmd),
